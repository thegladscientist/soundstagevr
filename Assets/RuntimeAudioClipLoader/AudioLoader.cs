...
namespace RuntimeAudioClipLoader
{
	...
	public class AudioLoader
	{
		...
		bool AudioClipSetDataOffsetSupported { get { return AudioLoaderConfig.IsWebGLPlayer; }}
		...
		/// <summary>
		/// Starts the process of loading AudioClip. Before calling this you can subscribe to events to make sure you dont miss any.
		/// </summary>
		public void StartLoading()
		{
			if (AudioClip == null) // AudioClip was destroyed
			{
				MyOnLoadingAborted();
				return;
			}
			if (Config.LoadMethod == LoadMethod.StreamInUnityThread) return; // no need to load data, they are being streamed
			if (reader == null) return; // already Disposed

			if (LoadState == AudioDataLoadState.Loading) return; // StartLoading() already called
			LoadState = AudioDataLoadState.Loading;
