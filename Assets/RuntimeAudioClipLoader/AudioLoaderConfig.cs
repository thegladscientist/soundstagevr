	...
	public class AudioLoaderConfig : ICloneable
	{
		...
		public static bool IsWebGLPlayer
		{
			get
			{
				...
				return false; // RuntimePlatform.WebGLPlayer was add in Unity 5.0.0
				#else
		...
		NativeOrManagerPreference preferredDecoder = NativeOrManagerPreference.PreferNative;
		/// <summary>
		/// Used to decide which decoder to use if both native and mannaged decoder are available. default: <see cref="RuntimeAudioClipLoader.NativeOrManagerPreference.PreferNative"/>
		/// </summary>
		public NativeOrManagerPreference PreferredDecoder { get { return preferredDecoder; } set { preferredDecoder = value; } }
		...
		{
			supportedFormats = new HashSet<string>(Enum.GetNames(typeof(SelectDecoder)).Select(s => s.ToLowerInvariant()));
		}
		...
		public static bool IsSupportedFormat(string filePath)
		{
			...
			return supportedFormats.Contains(GetExtension(filePath).ToLowerInvariant());
		}

		public static SelectDecoder GetAudioFormat(string filePath)
		{
			SelectDecoder ret = SelectDecoder.AutoDetect;
			try
			{
				ret = (SelectDecoder)System.Enum.Parse(typeof(SelectDecoder), GetExtension(filePath), true);
			}
			catch
			{
			}
