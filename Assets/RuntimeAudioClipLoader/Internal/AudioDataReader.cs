...
namespace RuntimeAudioClipLoader.Internal
{
	...
	public partial class AudioDataReader : WaveStream, ISampleProvider
	{
		...
		public static IMp3FrameDecompressor CreateMp3FrameDecoder(WaveFormat waveFormat, NativeOrManagerPreference preferredDecoder)
		{
			var useMannaged = false;

			if (IsNativeMp3DecoderAvailable && IsManagedMp3DecoderAvailable)
				useMannaged = preferredDecoder == NativeOrManagerPreference.PreferManaged;
			...
			if (useMannaged)
			{
				//return new NLayer.NAudioSupport.Mp3FrameDecompressor(waveFormat);
		...
		public AudioDataReader(Stream stream, SelectDecoder format, NativeOrManagerPreference preferredDecoder)
		{
		...
		WaveStream CreateReaderStream(Stream stream, SelectDecoder format, NativeOrManagerPreference preferredDecoder)
		{
